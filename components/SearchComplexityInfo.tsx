...
import { SearchAlgorithmInfo, SearchAlgorithmType, CellState } from '../types';
...
interface SearchComplexityInfoProps {
    info: SearchAlgorithmInfo;
    currentState: CellState;
    stepCount: number;
}

const getActiveLine = (algo: SearchAlgorithmType, state: CellState): number | null => {
    const lineMap: Record<string, Partial<Record<CellState, number>>> = {
        [SearchAlgorithmType.LinearSearch]: {
            [CellState.Checking]: 2,
            [CellState.Found]: 3,
            [CellState.Eliminated]: 4,
        },
        [SearchAlgorithmType.BinarySearch]: {
            [CellState.Checking]: 3,
            [CellState.Found]: 5,
            [CellState.Eliminated]: 6,
        },
...
const colorizeCode = (line: string): React.ReactNode => {
    const keywords = ['for', 'if', 'while', 'to', 'and', 'return', 'else'];
    const functions = ['binarySearch', 'linearSearch', 'jumpSearch', 'exponentialSearch', 'floor', 'sqrt', 'min'];
...
const SearchComplexityInfo: React.FC<SearchComplexityInfoProps> = ({ info, currentState, stepCount }) => {
    ...
    const handleAskAI = async () => {
        setShowAI(true);
        setLoading(true);
        // Using AlgorithmType-compatible string
        const result = await getAlgorithmComplexityAnalysis(info.name as any);
        setAnalysis(result);
        setLoading(false);
    };
        ...
        >
            ...
            <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 bg-[#091f2c] px-3 py-1.5 rounded-xl border border-white/[0.08]">
                    ...
                    <span className="text-[10px] text-white/35 uppercase tracking-wider font-semibold">Steps</span>
...
export default SearchComplexityInfo;
